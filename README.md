...
Where `command` could be one of `check`, `pipeline`, `prepare`, `candidate`, `fold`, and `recover`. (Run `python miR_PREFeR.py -h` to see help on options and commands.)
...
The predicted miRNA loci are written to a GFF file named NAME\_PREFIX + "_miRNA.gff3" in the output folder, where 'NAME\_PREFIX' is the prefix for naming output files which is specified in the configuration file.
