...
1. An HTML file that contains detailed information about each prediction:
a. Basic statistics of the predictions: the total number of predictions, the distribution of the length of the mature sequences, and the distribution of the first nucleotide of the first base of the mature sequences.
b. A table of detailed information about each predicted miRNA: the positions, the mature, star, precursor sequences, secondary structure, and detailed read mapping result of each input samples. For each predicted miRNA, the user can search the mature sequence on miRBASE by simply click a button. An example output HTML page can be found at <http://goo.gl/YQSCP8>.
...
4. A gff3 file that contains the predicted miRNAs.
