...
def prepare_data(dict_option, outtempfolder):
...
def gen_contig_typeA(expandedbam_plus, expandedbam_minus, dict_option, contig_minlen):
    ...
    try:
        ...
        output = awk_process.communicate()[0]
            ...
            sys.stderr.write(e)
            sys.stderr.write("Error occurred when generating contiguous(typeA).\n")
            sys.exit(-1)
...
def gen_miRNA_loci_nopredict(alndumpnames, rnalfoldoutnames, minlen,
...
def run_prepare(dict_option, outtempfolder, recovername):
    ...
    combinedbamname, expandedsamname, expandedbamname, expandedbam_plus, expandedbam_minus = prepare_data(dict_option, outtempfolder)
...
def check_loci_test(structures, matures, region, dict_aln, which):
    miRNAs = []
    for m0, m1, strand, mdepth in matures:
        # TODO Move this if to previous stage.
        # if mature length is not in [19-24], then ignore this
